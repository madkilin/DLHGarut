...
<section class="py-20 px-4 md:px-20 bg-white text-gray-800">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="keterangan">
            <p class="text-sm text-gray-500 mb-1">Di buat pada <b class="text-[#007546]">{{ $article->created_at->format('Y-m-d') }}</b></p>
            <p class="text-sm text-gray-500 mb-3">Oleh <b class="text-[#007546]">{{ $article->user->name }}</b></p>
        </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        fetch("{{ route('artikel.reward.check', $article->slug) }}")
            .then(res => res.json())
            .then(data => {
                if (data.claimed) {
                    ...
                    const countdown = setInterval(() => {
                        ...
                        if (timer <= 0) {
                            ...
                            fetch("{{ route('artikel.reward.claim', $article->slug) }}", {
                                    method: 'POST',
                                    headers: {
