            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 my-3">
                ...
                <form action="{{ route('exchange-point.store') }}" method="post">
                    ...
                    <div class="bg-white rounded-xl shadow-md p-4 text-center relative group">
                        ...
                        <p class="font-semibold text-lg mb-2 text-[#F17025]">Tersisa: {{ $reward->quota }}</p>

                        <input type="hidden" name="reward_id" value="{{ $reward->id }}">
                        <input type="hidden" name="point" value="{{ $reward->point }}">

                        <p class="text-yellow-600 text-sm mb-2">
                            Minimum: <strong>{{ $reward->point }}</strong> poin
                        </p>
                        ...
                        @else
                        <button type="button" class="bg-gray-500 text-white px-3 py-1 rounded-full">
                            Poin tidak cukup
                        </button>
                        @endif
                    </div>
