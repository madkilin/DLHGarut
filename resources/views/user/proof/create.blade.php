...
<section class="py-20 min-h-screen bg-gradient-to-br from-green-100 via-green-200 to-green-100">
    <div class="py-12 px-6 sm:px-10 max-w-3xl mx-auto bg-white text-black rounded-3xl shadow-lg">
        ...
        <form action="{{ route('proof.store', $complaint->id) }}" method="POST" enctype="multipart/form-data">
            @csrf

            <div class="mb-6">
                <label class="block font-semibold mb-2">Deskripsi</label>
                <textarea name="description" rows="4" class="w-full p-3 border rounded-xl focus:ring-2 focus:ring-green-400 focus:outline-none" required>{{ old('description') }}</textarea>
            </div>
        ...
        photoInput.addEventListener('change', function () {
            ...
            files.forEach(file => {
                ...
                reader.onload = function (e) {
                    ...
                    img.className = "w-24 h-24 object-cover rounded-lg cursor-pointer border-2 border-green-500";
