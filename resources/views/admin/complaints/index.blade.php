                                ...
                                <tr>
                                    ...
                                    <td class="px-4 py-2">
                                        <a href="{{ route('admin.complaints.show', $complaint->id) }}"
                                            class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 text-xs rounded-full">Detail</a>
                                        @if ($complaint->status === 'ditolak' || $complaint->status === 'selesai')
                                            <button onclick="openModal('modal-status-{{ $complaint->id }}')"
                                                class="bg-gray-500 hover:bg-gray-600 text-white px-3 py-1 text-xs rounded-full"
                                                disabled>Ubah
                                                Status</button>
                                        @else
                                            <button onclick="openModal('modal-status-{{ $complaint->id }}')"
                                                class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 text-xs rounded-full">Ubah
                                                Status</button>
                                        @endif

                                        @if ($complaint->status === 'diproses' && $complaint->assigned_to === null)
                                            <button onclick="openModal('modal-assign-{{ $complaint->id }}')"
                                                class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 text-xs rounded-full">Berikan
                                                Tugas</button>
                                        @endif
                                        ...
                                        @if ($complaint->status === 'selesai')
                                            <a href="{{ route('admin.complaints.print.complete', $complaint->id) }}"
                                                class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 text-xs rounded-full"
                                                target="_blank">
                                                Print seluruh data yang lengkap
                                            </a>
                                        @endif
                                    </td>
                                </tr>
