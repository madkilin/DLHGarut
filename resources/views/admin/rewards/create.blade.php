...
@section('content')
    <section class="py-16 bg-gradient-to-br from-green-100 via-green-200 to-green-100 min-h-screen">
        <div class="container mx-auto px-6 md:px-12 max-w-5xl bg-white text-black py-10 rounded-3xl">
            <h2 class="text-2xl font-bold text-[#F17025] mb-6">Tambah Hadiah</h2>
            <form action="{{ route('reward.store') }}" method="POST" enctype="multipart/form-data" class="text-black">
                @csrf
                <div class="mb-4">
                    <label for="name" class="block text-sm font-semibold">Nama Hadiah</label>
                    <input type="text" name="name" class="w-full border border-gray-300 rounded-lg px-4 py-2 mt-1" required>
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="quota" class="w-full border border-gray-300 rounded-lg px-4 py-2 mt-1" required>
                    @error('quota')
                        <div class="text-red-500">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="minimum" class="w-full border border-gray-300 rounded-lg px-4 py-2 mt-1" required>
                    @error('minimum')
                        <div class="text-red-500">
                            {{ $message }}
                        </div>
                ...
                <div class="flex justify-end space-x-2">
                    ...
                    <button type="submit" class="bg-[#007546] hover:bg-green-700 text-white px-4 py-2 rounded-lg">Simpan</button>
                </div>
            </form>
        </div>
    </section>
