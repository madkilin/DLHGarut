...
<section class="py-20 bg-gradient-to-br from-green-50 via-green-100 to-green-50 min-h-screen">
                ...
                <table id="articleTable" class="min-w-full">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <button onclick="openModal('modal-status-{{ $article->id }}')"
                                    class="focus:outline-none">
                                    @if ($article->is_read_by_admin == 1)
                                    <span
                                        class="inline-block bg-green-500 text-white px-4 py-1 rounded-full text-xs font-semibold">Konfirmasi</span>
                                    ...
                                    @else
                                    <span
                                        class="inline-block bg-yellow-500 text-white px-4 py-1 rounded-full text-xs font-semibold">Menunggu</span>
                                    @endif
                                </button>
                            </td>
    ...
    @foreach ($articles as $article)
    <!-- Modal Ubah Status -->
    <div id="modal-status-{{ $article->id }}"
        ...
        <div class="bg-white rounded-xl max-w-md w-full p-8 relative">
            <button onclick="closeModal('modal-status-{{ $article->id }}')"
            ...
            <form action="{{ route('article.update-status', $article->id) }}" method="POST"
                ...
                <div class="mt-6 flex justify-end space-x-4">
                    <button type="button" onclick="closeModal('modal-status-{{ $article->id }}')"
