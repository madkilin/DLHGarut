...
<section class="py-16 bg-gradient-to-br from-green-100 via-green-200 to-green-100 min-h-screen">
    <div class="container mx-auto px-6 md:px-12 max-w-5xl bg-white text-black py-10 rounded-3xl">
        ...
        <form action="{{ route('admin.articles.store') }}" method="POST" enctype="multipart/form-data" class="text-black">
            @csrf
            <div class="mb-4">
                <label for="title" class="block text-sm font-semibold">Title <span class="text-red-500">*</span></label>
                <input type="text" name="title" class="w-full border border-gray-300 rounded-lg px-4 py-2 mt-1"
                    required>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                <label for="description" class="block text-sm font-semibold">Deskripsi <span
                        class="text-red-500">*</span></label>
            ...
            <div class="flex justify-end space-x-2">
                ...
                <button type="submit"
                    class="bg-[#007546] hover:bg-green-700 text-white px-4 py-2 rounded-lg pointer">Simpan</button>
            </div>
        </form>
    </div>
</section>
