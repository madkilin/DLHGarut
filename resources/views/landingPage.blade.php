    ...
    <section id="home" class="relative bg-gradient-to-br from-green-100 via-green-200 to-green-100 py-24 overflow-hidden">
        <div class="container mx-auto flex flex-col-reverse md:flex-row items-center px-6 md:px-16">
            ...
            <div class="w-full md:w-1/2 text-center md:text-left mb-10 md:mb-0 animate-fade-up px-4">
                ...
                <div class="flex justify-center md:justify-start">
                    <a href="#explore" class="inline-block bg-[#F17025] hover:bg-[#F17025] text-white font-semibold px-6 py-3 rounded-full shadow-lg transition-all duration-300">Mulai
                        Jelajahi</a>
                </div>
            </div>
        ...
        <div class="absolute bottom-0 left-0 w-full overflow-hidden leading-none rotate-180" style="height: 80px;">
            <svg class="relative block w-full h-full" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 80" preserveAspectRatio="none">
                <path fill="#bbf7d0" d="M0,32L60,48C120,64,240,64,360,48C480,32,600,0,720,0C840,0,960,32,1080,53.3C1200,75,1320,85,1380,90L1440,96V0H1380C1320,0,1200,0,1080,0C960,0,840,0,720,0C600,0,480,0,360,0C240,0,120,0,60,0H0Z">
                </path>
            </svg>
        </div>
    </section>
    ...
    <section class="relative bg-gradient-to-b from-white to-green-50 text-gray-800 py-24 overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 ">
            ...
            <div class="relative overflow-hidden group">
                    ...
                    @forelse($articles as $key => $article)
                        <a href="{{ route('article.detail', $article->slug) }}">
                            <div class="card bg-white border border-[#007948] shadow-md w-64 flex-shrink-0 mx-2">
                                <figure>
                                    <img src="{{ asset('storage/' . $article->banner) }}" alt="{{ $article->title }}" class="w-64 h-40 object-cover object-center" />
                                </figure>
                                <div class="card-body">
                                    <h3 class="card-title text-lg text-[#007948]">{{ $article->title }}</h3>
                                    <p class="text-sm text-gray-600">
                                        {!! \Illuminate\Support\Str::limit(strip_tags($article->description), 50, '...') !!}
                                    </p>
                                </div>
                            </div>
                        </a>
                ...
                <!-- Tombol Navigasi -->
                <div class="absolute left-0 right-0 top-1/2 flex justify-between transform -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 px-6">
                    <button id="prevBtn" class="btn btn-circle bg-[#F17025]/80 text-white shadow-lg backdrop-blur-md hover:bg-[#F17025] border-2 border-white ring-1 ring-white/50">❮</button>
                    <button id="nextBtn" class="btn btn-circle bg-[#F17025]/80 text-white shadow-lg backdrop-blur-md hover:bg-[#F17025] border-2 border-white ring-1 ring-white/50">❯</button>
                </div>
                ...
                <table class="table-auto w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-[#F17025] bg-opacity-40">
                            ...
                            <th class="px-6 py-3 text-sm font-semibold uppercase tracking-wider">Lencana</th>
                            <th class="px-6 py-3 text-sm font-semibold uppercase tracking-wider">Level</th>
                            <th class="px-6 py-3 text-sm font-semibold uppercase tracking-wider">Exp</th>
                    ...
                    <tbody class="bg-white text-gray-800 rounded-lg overflow-hidden">
                        ...
                        @forelse($users as $key => $user)
                            ...
                            <tr class="hover:bg-[#F0FDF4]">
                                <td class="px-6 py-4 font-bold text-lg">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    <div class="flex">
                                        <img src="{{ asset($user->avatar) }}" alt="Profile" class="avatar-sm relative w-8 h-8 mr-3 rounded-full {{ $user->tier_border_class }}">
                                        <span>{{ ucfirst($user->name) }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">{{ $user->tier_icon . ' - ' . ucFirst($user->tier) }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" aliggn="center">Belum ada user</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
...
@section('script')
    ...
    <script>
        ...
        carousel.addEventListener('click', (e) => {
            if (isDragging) {
                e.preventDefault(); // blok klik kalau sedang drag
            }
        });
